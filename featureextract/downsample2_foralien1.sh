...
height=1080
		...
		do
		../H265EncoderD -c encoder.cfg -quality_level $quality -rc $rcmode -crf $crf -pass $pass -fr $framerate -frames $framenum -w $width -h $height -keyInt $IPeriod -b ./${sequence2}_1920x1080_HW_Q${quality}_CRF${crf}.${bin_postfix} -i ${org_path}/${sequence1}.yuv 1>./encod_${sequence1}__1920x1080_HW_Q${quality}_CRF${crf}.txt		
		done	
done
...
height=1080
		...
		for crf in `seq 10 42`
		do
		../H265EncoderD -c encoder.cfg -quality_level $quality -rc $rcmode -crf $crf -pass $pass -fr $framerate -frames $framenum -w $width -h $height -keyInt $IPeriod -b ./${sequence2}_HW_Q${quality}_CRF${crf}.${bin_postfix} -i ${org_path}/${sequence2}.yuv 1>./encod_${sequence2}_HW_Q${quality}_CRF${crf}.txt		
		done	
done
...
height=1080
		...
		for crf in `seq 10 42`
		do
		../H265EncoderD -c encoder.cfg -quality_level $quality -rc $rcmode -crf $crf -pass $pass -fr $framerate -frames $framenum -w $width -h $height -keyInt $IPeriod -b ./${sequence3}_HW_Q${quality}_CRF${crf}.${bin_postfix} -i ${org_path}/${sequence3}.yuv 1>./encod_${sequence3}_HW_Q${quality}_CRF${crf}.txt		
		done
done				
...
height=1080
		...
		for crf in `seq 10 42`
		do
		../H265EncoderD -c encoder.cfg -quality_level $quality -rc $rcmode -crf $crf -pass $pass -fr $framerate -frames $framenum -w $width -h $height -keyInt $IPeriod -b ./${sequence4}_HW_Q${quality}_CRF${crf}.${bin_postfix} -i ./${sequence4}.yuv 1>${log_path}/encod_${sequence4}_HW_Q${quality}_CRF${crf}.txt		
		done
